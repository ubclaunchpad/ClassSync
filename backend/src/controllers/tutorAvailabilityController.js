...
export default class tutorAvailabilityController {
  ...
  async getAppointmentsByTutor(tutor_id, date) {
    return this.tutor
      .getAppointmentsByTutor(tutor_id, date)
      .then((result) => {
        return result;
      })
      .catch((err) => {
  ...
  async getTutorByEnrollment(enrollment_id) {
    return this.tutor
      .getTutorByEnrollment(enrollment_id)
      .then((tutors) => {
        return tutors;
      })
      .catch((err) => {
  ...
  async getAvailableTutors(startDate, course_id) {
    return this.tutor
      .getAvailableTutors(startDate, course_id)
      .then((availabilityData) => {
        ...
        //           Object.entries(tutorData.availability_times).forEach(
        //             ([day, timeSlots]) => {
        ...
        availabilityHashmap = Object.fromEntries(
          Object.entries(availabilityHashmap).filter(
            ([day, timeSlots]) => Object.keys(timeSlots).length > 0
          )
        );
  ...
  async getSelectedTutorsAvailabilityByTime(
    startDate,
    tutor_ids,
    day,
    time1,
    ...
    return this.tutor
      .getSelectedTutorsAvailability(startDate, tutor_ids)
      .then((availabilityData) => {
        const filteredData = availabilityData.filter(
          (item) =>
  ...
  async getRecurringAvailability(userID) {
    return this.tutor
      .getRecurringAvailability(userID)
      .then((availability) => {
        return availability;
      })
      .catch((err) => {
  ...
  async clearAvailability(userID, startDate) {
    return this.tutor
      .clearAvailability(userID, startDate)
      .then((result) => {
        return result;
      })
      .catch((err) => {
  ...
  async resetAvailability(userID, startDate, endDate) {
    return this.tutor
      .resetAvailability(userID, startDate, endDate)
      .then((result) => {
        return result;
      })
      .catch((err) => {
  ...
  async removeAvailability(userID, startDate, endDate, day, times) {
    return this.tutor
      .removeAvailability(userID, startDate, endDate, day, times)
      .then((result) => {
        return result;
      })
      .catch((err) => {
  ...
  async addSlots(userID, startDate, endDate, day, times) {
    return this.tutor
      .addSlots(userID, startDate, endDate, day, times)
      .then((result) => {
        return result;
      })
      .catch((err) => {
  ...
  async setAvailability(userID, weeks, availability, isRecurring) {
    try {
      ...
      const patternID = await this.tutor.createAvailabilityPattern(
        availability
      );
      ...
      weeks.forEach(async (curWeek) => {
        const week = JSON.parse(curWeek);

        const appts = await this.tutor.getAppointmentsByTutorDates(
          userID,
          week.start_date,
          week.end_date
        );
        ...
        if (appts.length == 0) {
          await this.tutor.setAvailabilityForWeeks(userID, [week], patternID);
        } else {
          console.log("APPTS != 0\n");
          ...
          appts.forEach((appt) => {
            ...
            // check availability for day -> if availabiility contains the selected time, remove them
            const times = [selectedTime, thirtyMinsLater];

            if (
              newAvailability[day].includes(selectedTime) ||
              newAvailability[day].includes(thirtyMinsLater)
            ) {
          ...
          const newPatternID = await this.tutor.createAvailabilityPattern(
            newAvailability
          );

          await this.tutor.setAvailabilityForWeeks(
            userID,
            [week],
            newPatternID
          );
  ...
  getAvailability(userID) {
    ...
    return this.tutor
      .getTutorAvailability(userID, today)
      .then((availability) => {
  ...
  //       const appts = await this.tutor.getAppointmentsByTutor(
  //         userID,
  //         week.start_date,
  //         week.end_date
  //       );
