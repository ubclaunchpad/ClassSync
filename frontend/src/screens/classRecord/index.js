...
const ClassRecordForm = () => {
  ...
  useEffect(() => {
    ...
    const getSharedFiles = async () => {
      const response = await fetch(`http://localhost:8080/sharedFiles?id=${id}`);
...
const handleFileDownload = async () => {
  ...
  flexWrap: 'wrap', // Add flexWrap to wrap
    ...
    <div style={{ display: 'flex', alignItems: 'center', margin: '5px 0', width: '100%', justifyContent: 'center' }}>
      ...
      <svg style={{ marginRight: '10px' }} xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"/><path d="M17 12h-5v5h5v-5zM16 1v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-1V1h-2zm3 18H5V8h14v11z"/></svg>
        ...
        {classInfo.start_time != null  && new Date(classInfo.start_time).toLocaleString('en-CA', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}    </div>
  ...
  <div style={{ width: '50%' }}>
    ...
    <svg style={{ marginRight: '10px' }} xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24" height="24" viewBox="0 0 24 24" width="24">
      ...
      <a href={`https://${classInfo.link}`} style={{ marginRight: '10px' }}>      
      Meeting Link
      </a>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', margin: '5px 0' }}>
...
<a href={`/course/${classInfo.enrollment_id}`}>
Lesson Plan
</a>
</div>
   </div>
    ...
    <div style={{ display: 'flex', height: '20px', marginTop: '5px', minWidth:'100%'}}>
      {Array.from({ length: classDetails.length - 1 }).map((_, index) => (
        <div key={index} style={{ flex: 1, backgroundColor: '#103da2', borderRight: '1px solid white' }}></div>
      ))}
      <div style={{ flex: 1, backgroundColor: '#00B0F1', borderRight: '1px solid white' }}></div>
      {Array.from({ length: 5 - classDetails.length }).map((_, index) => (
        <div key={index + classDetails.length} style={{ flex: 1, backgroundColor: 'lightgrey', borderRight: '1px solid white' }}></div>
      ))}
  </div>
  ...
  <div style={{
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', marginBottom: '10px', paddingLeft: '10px'}}>
        <input type="checkbox" checked={fileList != null && selectedFiles.length === fileList.length}onChange={handleSelectAll} style={{ marginRight: '10px' }} />
        {
          (selectedFiles.length === 0 && (!sharedFiles || sharedFiles.length === 0)) ? 
          <button 
            style={{ marginRight: 'auto', marginLeft: '5px', backgroundColor: '#d3d3d3', border: 'none', borderRadius: '3px', padding: '5px 10px', color: '#fff' }}
            disabled
          >
            Share
          </button>
          ...
          <button 
            ...
            style={{ marginRight: 'auto', marginLeft: '5px', backgroundColor: '#1890ff', border: 'none', borderRadius: '3px', padding: '5px 10px', color: '#fff' }}
          >
      ...
      <ul style={{ listStyle: 'none', padding: 0}}>
        {fileList != null && fileList.length > 0 && fileList.map((file) => (
      <li 
      key={file.name} 
      style={{ 
        marginBottom: '10px', 
    ...
    >
      ...
      />
      <span > 
      <div key={JSON.stringify(sharedFiles)} style={{ display: 'inline-block' }}>
      {sharedFiles != null && sharedFiles.length > 0 && sharedFiles.some(selectedFile => selectedFile.name === file.name) && (
        
        <svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24" height="16" viewBox="0 0 24 24" width="16"><g><rect fill="none" height="24" width="24"/></g><g><g/><g><g><path d="M16.67,13.13C18.04,14.06,19,15.32,19,17v3h4v-3 C23,14.82,19.43,13.53,16.67,13.13z" fill-rule="evenodd"/></g><g><circle cx="9" cy="8" fill-rule="evenodd" r="4"/></g><g><path d="M15,12c2.21,0,4-1.79,4-4c0-2.21-1.79-4-4-4c-0.47,0-0.91,0.1-1.33,0.24 C14.5,5.27,15,6.58,15,8s-0.5,2.73-1.33,3.76C14.09,11.9,14.53,12,15,12z" fill-rule="evenodd"/></g><g><path d="M9,13c-2.67,0-8,1.34-8,4v3h16v-3C17,14.34,11.67,13,9,13z" fill-rule="evenodd"/></g></g></g></svg>
         )}
         </div>
            {fileIcons[file.name.split('.').pop()] || fileIcons.default}</span>            
      <span 
        style={{ 
          marginLeft: '8px',
...
<Card>
      ...
      <Tabs
        ...
        variant="fullWidth"
      >
        {classDetails.length > 0 && [...Array(classDetails.length)].map((_, index) => (
          <Tab
            key={index}
            label={`Class ${index + 1}`}
            value={index}
          ...
          <div style={{ 
            ...
            {selectedTab == (classDetails.length > 0 ? classDetails.length - 1 : 0) ?
        (    <form noValidate autoComplete="off" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'left', width: '45%' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', color: '#103da2', marginBottom: '20px' }}>
                <p style={{ marginBottom: '0' }}>Tutor: {classInfo.tutor}</p>  {/* Replace with actual tutor name */}
                <p style={{ marginBottom: '0' }}>   {new Date(classInfo.start_time).toLocaleDateString('en-GB', {
                      day: '2-digit',
                      month: 'short',
                      year: 'numeric',
                    })}</p>  {/* Replace with actual date */}
              </div>
                {classDetails.length > 0 && Object.entries(classDetails[selectedTab].notes).map(([key, value], index) => (
                  <div key={index}>
                    <p>{key}</p>
                    <TextField
              ...
              <Button variant="contained" color={saveText === 'Saved' ? 'secondary' : 'primary'} style={{ marginTop: '20px' }} onClick={() => {
  setSaveText("Saving..");
  handleSave();
}}>
    {saveText}
              ...
              <div style={{ width: '45%', maxWidth: '600px' }}>
                <div key={classDetails} style={{ display: 'flex', justifyContent: 'space-between', color: '#103da2', marginBottom: '20px' }}>
                  {classDetails[selectedTab] ? (
                    <>
                      <p style={{ marginBottom: '0' }}>Tutor: {classDetails[selectedTab].tutor}</p>
                      <p style={{ marginBottom: '0' }}>
                        {new Date(classDetails[selectedTab].start_time).toLocaleDateString('en-GB', {
                          day: '2-digit',
                          month: 'short',
                          year: 'numeric',
                        })}
                ...
                <div style={{ textAlign: 'left' }}>
                  {classDetails.length > 0 && classDetails[selectedTab] && Object.entries(classDetails[selectedTab].notes).map(([key, value], index) => (
                    <div key={index}>
                      <p>{key}</p>
